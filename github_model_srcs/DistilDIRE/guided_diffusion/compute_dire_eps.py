...
@torch.no_grad()
def dire(img_batch:torch.Tensor, model, diffusion, args, device, save_img=False, save_path=None):
...
if __name__ == "__main__":
    ...
    os.makedirs(osp.join(adm_args['save_root'], 'images', 'fake'), exist_ok=True)
    os.makedirs(osp.join(adm_args['save_root'], 'images', 'real'), exist_ok=True)
    os.makedirs(osp.join(adm_args['save_root'], 'dire', 'fake'), exist_ok=True)
    os.makedirs(osp.join(adm_args['save_root'], 'dire', 'real'), exist_ok=True)
    os.makedirs(osp.join(adm_args['save_root'], 'eps', 'fake'), exist_ok=True)
    os.makedirs(osp.join(adm_args['save_root'], 'eps', 'real'), exist_ok=True)
    print(f"Dataset length: {len(dataset)}")
    dataloader = DataLoader(dataset, batch_size=adm_args['batch_size'], num_workers=2, drop_last=False, shuffle=False)
    ...
    for (img_batch, dire_batch, eps_batch, isfake_batch), (img_pathes, dire_pathes, eps_pathes) in tqdm(dataloader):
        ...
        for i in range(len(img_batch)):
            ...
            img_path = osp.join(adm_args['save_root'], 'images', 'fake', basename) if isfake else osp.join(adm_args['save_root'], 'images', 'real', basename)
        ...
        with torch.no_grad():
            ...
            if adm_args['compute_dire']:
                dire_img, img, recons = dire(img_batch, adm_model, diffusion, adm_args, device)
